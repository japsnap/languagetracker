...
- **setCachedExtra WHERE clause aligned with findCachedWordRow** — `setCachedExtra` previously matched rows by `(input_word, input_language, learning_language, primary_language, mode)`. `findCachedWordRow` locates rows by `(result_word OR input_word, learning_language, primary_language, mode)` — no `input_language`. These diverged, causing UPDATE to miss the row that was just found. Fix: `setCachedExtra` WHERE clause now uses `result_word + learning_language + primary_language + mode` (identical to `findCachedWordRow`). `inputLang` param kept for call-site compatibility but is no longer used in the WHERE. Callers must pass the AI-corrected result word (e.g. `wordLower` from `fetchInsights`) as the first argument.

- **ai_insights cache key mismatch — structural fix** — Root cause: `word_cache.input_word` stores the user's raw typed text (e.g. `"grinder"`, `"buenisima"`), but vocabulary stores the AI-corrected result word (e.g. `"molinillo"`, `"buenísima"`). `findCachedWordRow` was searching `input_word = word.word` — always a miss for corrected/translated words. Fix: added `result_word` to `CACHE_INDEXED_FIELDS` in `cache.js`, aliased from `response.word` via new `CACHE_INDEXED_ALIASES = { result_word: 'word' }`. All `setCachedWord` calls now populate `result_word` from the AI-corrected output. `findCachedWordRow` queries `result_word OR input_word` (Supabase `.or()`) so it finds rows regardless of whether the user's typed text matches the result. Returns `{ inputLang, inputWord, cacheData }` — `inputWord` is the actual DB-stored `input_word` value used by `setCachedExtra` for the UPDATE WHERE clause. `insights.js` updated to use `cacheMatch.inputWord` (not `wordLower`) when calling `setCachedExtra`. SQL migration required: `ALTER TABLE word_cache ADD COLUMN IF NOT EXISTS result_word text;`

## 2026-04-20

- **ai_insights cache key mismatch fixed** — Root cause: `setCachedExtra` was building the UPDATE key with `input_word = word.word` and `input_language = word.word_language`, but the cache row was stored with `input_word = original_typed_word` and `input_language = original_input_lang` (e.g. user typed "beautiful" in English → cache row has `input_word="beautiful", input_language="en"`, but insights tried to UPDATE with `input_word="hermoso", input_language="es"` — 0 rows matched). Fix: new `findCachedWordRow(word, learningLang, primaryLang, mode)` in `cache.js` queries WITHOUT `input_language`, finds the row and returns its actual stored `input_language`. `fetchInsights` uses this to build the correct UPDATE key. If no cache row exists at all (cross-language words where `input_word != result word`), the cache write is skipped gracefully and `vocabulary.ai_insights` still receives the write. Console logs on all paths (hit, miss, stored key values) for verification.
